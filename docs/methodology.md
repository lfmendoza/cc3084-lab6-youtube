...
| Fuente del PDF | base-14 (Helvetica), DejaVu, FreeSans, Liberation | **La primera familia del sistema que aporte las cuatro caras (regular, negrita, cursiva y negrita-cursiva) y supere una comprobación de cobertura de glifos.** Las dos condiciones son necesarias: sin las cuatro caras, `registerFontFamily` no puede mapear `<b>` ni `<i>` y **el énfasis en línea desaparece sin aviso** (defecto detectado en la revisión visual del PDF: DejaVu no incluye la cara oblicua en esta distribución, y todos los `**negrita**` se estaban perdiendo); y Helvetica no tiene glifos para κ, Δ, ≥ ni las comillas angulares, que aparecerían como cuadros vacíos. El orden de preferencia es FreeSans → DejaVu → Liberation → base-14. Los emojis se describen por nombre y punto de código porque ninguna fuente incrustable cubre los planos suplementarios. |
